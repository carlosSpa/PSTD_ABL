...
void ricker_wavelet_tis0(domain* D,float dt,int nt,float t0,float f0,int l){
    ...
    D->sourceB=(float*)malloc((D->nt)*sizeof(float));
	...
	if(l==0){
		...
		for(int t=0;t<D->nt;t++){
			if(t!=0)
				D->source[t]=100*((1-2*B*(t*D->Dt-t0-1)*(t*D->Dt-t0-1))*exp(-B*(t*D->Dt-t0-1)*(t*D->Dt-t0-1))
						+(1-2*B*(t*D->Dt-t0-2)*(t*D->Dt-t0-2))*exp(-B*(t*D->Dt-t0-2)*(t*D->Dt-t0-2))
						+(1-2*B*(t*D->Dt-t0-3)*(t*D->Dt-t0-3))*exp(-B*(t*D->Dt-t0-3)*(t*D->Dt-t0-3)));
	...
	if(type==0){
        ...
        if(t==1){
            for(int n=0;n<D->nt;n++){
                for(int k=0;k<n;k++){
                    D->sourceB[n]=D->sourceB[n]+D->source[k];
                }
            }
        }
		for(int i=-1;i<2;i++){
			for(int j=-1;j<2;j++){
				for(int k=-1;k<2;k++){
					D->pxn[nS+i+j*D->nx+k*D->nx*D->ny]=-D->sourceB[D->nt-1-t]*exp(-1*((i)*(i)+(j)*(j)+(k)*(k)))+D->pxn[nS+i+j*D->nx+k*D->nx*D->ny];
					D->pyn[nS+i+j*D->nx+k*D->nx*D->ny]=-D->sourceB[D->nt-1-t]*exp(-1*((i)*(i)+(j)*(j)+(k)*(k)))+D->pyn[nS+i+j*D->nx+k*D->nx*D->ny];
					D->pzn[nS+i+j*D->nx+k*D->nx*D->ny]=-D->sourceB[D->nt-1-t]*exp(-1*((i)*(i)+(j)*(j)+(k)*(k)))+D->pzn[nS+i+j*D->nx+k*D->nx*D->ny];
